#!/usr/bin/env python3
"""
Replay OpenAI Batch-style embedding request shards directly against /v1/embeddings.

This is the embedding analogue of `run_openai_requests_via_gateway.py`, but it talks
to the OpenAI embeddings endpoint directly and writes batch-shaped result shards so the
existing rehydration code can consume them unchanged.
"""
...
def _parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Replay embedding request shards directly against OpenAI /v1/embeddings.")
    ...
    ap.add_argument("--max-concurrency", type=int, default=16, help="Thread count for concurrent embedding calls")
    ap.add_argument("--max-retries", type=int, default=4, help="Retry count for transient failures")
    ap.add_argument("--resume", action="store_true", help="Skip custom_ids already written to result/error shards")
    ap.add_argument(
        "--shard-indices",
        default="",
        help=(
    ...
    for path in req_paths:
        ...
        for raw in path.read_text(encoding="utf-8").splitlines():
            ...
            if obj.get("url") != "/v1/embeddings":
                raise SystemExit(f"Request line in {path} is not an embeddings request")
            model = body.get("model")
            text = body.get("input")
            if not isinstance(model, str) or not model:
                raise SystemExit(f"Embeddings request in {path} missing model")
            if not isinstance(text, str):
                raise SystemExit(f"Embeddings request in {path} missing input text")
            out.append(WorkItem(shard=shard, custom_id=cid, model=model, text=text))
    return out


def _embedding_result_line(*, custom_id: str, model: str, response_body: dict[str, Any]) -> dict[str, Any]:
    return {
...
def _embedding_error_line(*, custom_id: str, model: str, status_code: int, message: str) -> dict[str, Any]:
    return {
        "id": _stable_fake_id(custom_id, model=model),
        "custom_id": custom_id,
        "response": {
            ...
            "request_id": "",
            "body": {"error": {"message": str(message), "type": "embedding_error"}},
        },
        "error": None,
    }
...
def _call_embedding_api(*, api_base: str, api_key: str, item: WorkItem, timeout: float, max_retries: int) -> tuple[bool, dict[str, Any]]:
    url = api_base.rstrip("/") + "/v1/embeddings"
    payload = {"model": item.model, "input": item.text}
    ...
    for attempt in range(max(1, int(max_retries))):
        ...
        try:
            with urllib.request.urlopen(req, timeout=timeout) as resp:
                ...
                return True, _embedding_result_line(custom_id=item.custom_id, model=item.model, response_body=body)
            ...
            if attempt + 1 < max_retries and _should_retry(exc.code):
                ...
                backoff *= 2.0
                continue
            return False, _embedding_error_line(custom_id=item.custom_id, model=item.model, status_code=exc.code, message=message)
        except Exception as exc:  # noqa: BLE001
            if attempt + 1 < max_retries:
                time.sleep(backoff)
                backoff *= 2.0
                continue
            return False, _embedding_error_line(custom_id=item.custom_id, model=item.model, status_code=599, message=str(exc))
    return False, _embedding_error_line(custom_id=item.custom_id, model=item.model, status_code=599, message="embedding request exhausted retries")
...
def main() -> None:
    ...
    try:
        ...
        (output_dir / "sync_embedding_summary.json").write_text(json.dumps(summary, indent=2) + "\n", encoding="utf-8")
