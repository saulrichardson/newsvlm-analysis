#!/usr/bin/env python3
"""
Cluster issue embeddings with UMAP → HDBSCAN (\"tweet topics\" style).

Inputs (embedding_dir)
  - issue_ids.txt
  - issue_embeddings.npy
  - issue_metadata.jsonl (optional but recommended)

Outputs (output_dir)
  - clusters.jsonl: per-issue cluster assignment + probability
  - cluster_summary.json: cluster sizes + basic stats
  - umap_2d.npy: 2D projection aligned with issue_ids.txt (for plotting)
...
def _load_issue_ids(path: Path) -> list[str]:
    if not path.is_file():
        raise SystemExit(f"Missing issue_ids.txt: {path}")
    ...
    with path.open("r", encoding="utf-8") as f:
        for raw in f:
            ...
            if isinstance(issue_id, str) and issue_id.strip():
                out[issue_id.strip()] = obj
    return out


def _parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Cluster issue embeddings with UMAP→HDBSCAN.")
    ap.add_argument("--embedding-dir", required=True, help="Directory containing issue_embeddings.npy + issue_ids.txt")
...
def main() -> None:
    ...
    issue_ids = _load_issue_ids(emb_dir / "issue_ids.txt")
    X = np.load(emb_dir / "issue_embeddings.npy")
    if X.ndim != 2:
        raise SystemExit(f"Expected 2D embedding matrix; got shape {X.shape} from {emb_dir/'issue_embeddings.npy'}")
    if X.shape[0] != len(issue_ids):
        raise SystemExit(f"Row mismatch: embeddings rows={X.shape[0]} issue_ids={len(issue_ids)}")

    meta = _load_metadata(emb_dir / "issue_metadata.jsonl")
    ...
    # Per-issue cluster assignments.
    if len(issue_ids) != len(labels) or len(issue_ids) != len(probs):
        raise SystemExit(
            f"Length mismatch: issue_ids={len(issue_ids)} labels={len(labels)} probs={len(probs)} "
    ...
    with clusters_path.open("w", encoding="utf-8") as f:
        for issue_id, lab, prob in zip(issue_ids, labels.tolist(), probs.tolist()):
            row = {
                "issue_id": issue_id,
                "cluster_id": int(lab),
                "cluster_prob": float(prob),
                **({"issue_date": meta.get(issue_id, {}).get("issue_date")} if meta else {}),
                **({"classification_label": meta.get(issue_id, {}).get("classification_label")} if meta else {}),
            }
    ...
    # Also write a simple frequency-by-year table for this clustering (helps debugging).
    by_year = defaultdict(Counter)
    for issue_id, lab in zip(issue_ids, labels.tolist()):
        d = meta.get(issue_id, {}).get("issue_date")
    ...
    print(f"Done. wrote={out_dir} clusters={summary['n_clusters']} noise={summary['n_noise']}/{summary['n_issues']}")
