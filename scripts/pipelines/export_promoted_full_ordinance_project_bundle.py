...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Export promoted hidden full ordinances into a Desmond-style project folder "
            "with metadata.csv at root, clean ordinances in parsed/, raw page PNGs in "
            "raw/<issue_id>/, and OCR transcripts in transcript/."
        )
    )
    parser.add_argument("--promoted-csv", type=Path, required=True)
    parser.add_argument("--excerpt-dataset-dir", type=Path, required=True)
    parser.add_argument("--amend-dataset-dir", type=Path, required=True)
    parser.add_argument("--excerpt-results-root", type=Path, required=True)
    parser.add_argument("--amend-results-root", type=Path, required=True)
...
def write_readme(target_root: Path, *, exported_count: int, skipped_count: int) -> None:
    text = (
        "# Promoted Hidden Full Ordinances\n\n"
        "This folder contains hidden full ordinances promoted out of non-full-code classifier labels.\n\n"
        "Layout:\n"
        "- `metadata.csv`: one row per exported promoted issue\n"
        "- `parsed/<issue_id>.md`: clean promoted ordinance text\n"
        ...
        f"Promoted issues skipped because no clean ordinance text was available: `{skipped_count}`\n"
    )
...
def main() -> None:
    args = parse_args()
    promoted_csv = args.promoted_csv.expanduser().resolve()
    excerpt_dataset_dir = args.excerpt_dataset_dir.expanduser().resolve()
    amend_dataset_dir = args.amend_dataset_dir.expanduser().resolve()
    excerpt_results_root = args.excerpt_results_root.expanduser().resolve()
    amend_results_root = args.amend_results_root.expanduser().resolve()
    ...
    if not promoted_csv.is_file():
        raise SystemExit(f"--promoted-csv not found: {promoted_csv}")
    for path in [
        excerpt_dataset_dir,
        amend_dataset_dir,
        excerpt_results_root,
        amend_results_root,
    ]:
        if not path.exists():
    ...
    excerpt_issues, excerpt_class = load_dataset_rows(excerpt_dataset_dir)
    amend_issues, amend_class = load_dataset_rows(amend_dataset_dir)
    excerpt_tx = load_transcriptions(excerpt_results_root)
    amend_tx = load_transcriptions(amend_results_root)

    dataset_issue_index = {
        "code_publication_excerpt_or_installment": excerpt_issues,
        "amendment_substantial_text": amend_issues,
    }
    class_index = {
        "code_publication_excerpt_or_installment": excerpt_class,
        "amendment_substantial_text": amend_class,
    }
    tx_index = {
        "code_publication_excerpt_or_installment": excerpt_tx,
        "amendment_substantial_text": amend_tx,
    }

    promoted_rows = [
        row
        for row in load_csv_rows(promoted_csv)
        if clean(row.get("second_stage_class")) == "full_ordinance_text"
    ]
    if not promoted_rows:
        raise SystemExit(f"No promoted full ordinances found in {promoted_csv}")

    metadata_rows: list[dict[str, Any]] = []
    skipped_rows: list[dict[str, Any]] = []
    for promo in sorted(promoted_rows, key=lambda row: clean(row.get("issue_id"))):
        issue_id = clean(promo.get("issue_id"))
        source_label = clean(promo.get("source_label"))
        if source_label not in dataset_issue_index:
            raise SystemExit(f"Unsupported source_label={source_label!r} for issue_id={issue_id}")

        issue_row = dataset_issue_index[source_label].get(issue_id)
        class_row = class_index[source_label].get(issue_id)
        tx_row = tx_index[source_label].get(issue_id)
        ...
        row = {
                ...
                "promotion_second_stage_class": clean(promo.get("second_stage_class")),
    ...
    if not metadata_rows:
        raise SystemExit("No promoted issues with clean transcription outputs were exportable")
    ...
    write_readme(target_root, exported_count=len(metadata_rows), skipped_count=len(skipped_rows))

    skipped_path = target_root / "skipped_promoted_missing_clean_text.csv"
    if skipped_rows:
